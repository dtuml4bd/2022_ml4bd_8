...
checker=0;
#attribute
message = tk.Label(window, text="DTU UNIVERSITY" ,bg="white"  ,fg="black"  ,width=20  ,height=2,font=('Times New Roman', 25, 'bold')) 
message.place(x=1150, y=698)

message = tk.Label(window, text="Driver Drowsiness Detection System" ,bg="pink"  ,fg="black"  ,width=40  ,height=1,font=('Times New Roman', 35, 'bold underline')) 
...
lbl4.place(x=190-x_cord, y=375-y_cord)
...
lbl6.place(x=930-x_cord, y=362-y_cord)

lbl6 = tk.Label(window, text="STEP 4",width=20  ,fg="green"  ,bg="pink"  ,height=2 ,font=('Times New Roman', 20, ' bold ')) 
lbl6.place(x=1200-x_cord, y=362-y_cord)
#clear id
def clear1():
    txt.delete(0, 'end')    
    res = ""
...
def TakeImages():
    if txt.get() == "" or txt2.get() == "" :
            messagebox.showwarning("Empty Field", "All fields are required", parent = window)
    else:
            try:
                ...
                #use the cursor method of mysqlconnection object to perform                           
                curs = connection.cursor()
                #the execute method runs query
                curs.execute("insert into studentdetail (id,name) values(%s,%s)",
                                        (
                                            txt.get(),
                                            txt2.get(),
    ...
    if not Id:
        ...
        if MsgBox == 'no':
            tk.messagebox.showinfo('Your need','Please')
    elif not name:
        ...
        if MsgBox == 'no':
            ...
            # video from a connected camera
            cam = cv2.VideoCapture(0)
            #use Haar cascade to dectect face
            ...
            while(True):
                ...
                #when faces are detected return rect(x,y,w,h) of face
                for (x,y,w,h) in faces:
                    cv2.rectangle(img,(x,y),(x+w,y+h),(255,0,0),2)        
                    #incrementing sample number 
                    sampleNum=sampleNum+1
                    #saving the captured face in the dataset folder TrainingImage
                    #This particular code will return the cropped face from the image.
...
def TrainImages():
    #create model recogizer face
    recognizer = cv2.face_LBPHFaceRecognizer.create()
    faces,Id = getImagesAndLabels("TrainingImage")
    recognizer.train(faces, np.array(Id))
    #save model into file yml
...
def TrackImages():
    ...
    with open('Attendance\Total.csv','a+') as csvFile:
        writer = csv.writer(csvFile)
        writer.writerow(row)
    csvFile.close()
...
def quit_window():
   ...
   if MsgBox == 'yes':
       ...
       window.destroy()
#method    
takeImg = tk.Button(window, text="IMAGE CAPTURE ", command=TakeImages  ,fg="white"  ,bg="blue"  ,width=15  ,height=2, activebackground = "pink" ,font=('Times New Roman', 15, ' bold '))
takeImg.place(x=245-x_cord, y=425-y_cord)
trainImg = tk.Button(window, text="MODEL TRAINING ", command=TrainImages,fg="white"  ,bg="blue"  ,width=15  ,height=2, activebackground = "pink" ,font=('Times New Roman', 15, ' bold '))
trainImg.place(x=645-x_cord, y=425-y_cord)
trackImg = tk.Button(window, text="ATTENDANCE", command=TrackImages  ,fg="white"  ,bg="red"  ,width=15  ,height=2, activebackground = "pink" ,font=('Times New Roman', 15, ' bold '))
trackImg.place(x=1000-x_cord, y=425-y_cord)
# driver
dectedImage = tk.Button(window, text="DROWSINESS", command=function1 ,fg="white"  ,bg="red"  ,width=15  ,height=2, activebackground = "pink" ,font=('Times New Roman', 15, ' bold '))
dectedImage.place(x=1260-x_cord, y=425-y_cord)
# 
