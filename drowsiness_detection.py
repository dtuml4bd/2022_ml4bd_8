...
EAR_THRESHOLD = 0.2
#số  lượng khung ảnh khi buồn ngủ cần thiết để kích hoạt cảnh báo
CONSECUTIVE_FRAMES = 20
#tỉ lệ khuôn miệng (khi buồn ngủ)
MAR_THRESHOLD = 16

#biến đếm số lần nháy mắt và số khung
BLINK_COUNT = 0 
FRAME_COUNT = 0 

# Now, intialize the dlib's face detector model as 'detector' and the landmark predictor model as 'predictor'
# dùng model dlib dò tìm khuôn mặt để xác định các điểm mốc trên mắt & miệng
print("[INFO]Loading the predictor.....")
detector = dlib.get_frontal_face_detector() 
predictor = dlib.shape_predictor(args["shape_predictor"])

# Grab the indexes of the facial landamarks for the left and right eye respectively
# toạ độ của mắt và miệng
...
#tạo folder chứa ảnh ngủ và ngáp
...
# NHẬN DIỆN
while True: 
	# Extract a frame - trích xuất
	frame = vs.read()
	cv2.putText(frame, "PRESS 'q' TO EXIT", (10, 30), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 0, 255), 3) 
	# Resize the frame 
	frame = imutils.resize(frame, width = 500)
	# Convert the frame to grayscale - chuyển sang ảnh trắng đen
	gray = cv2.cvtColor(frame, cv2.COLOR_BGR2GRAY)
	# Detect faces - xác định mặt mặt
	rects = detector(frame, 1)

	# Now loop over all the face detections and apply the predictor 
	# hiển thị cái landmark
	for (i, rect) in enumerate(rects): 
		shape = predictor(gray, rect)
		# Convert it to a (68, 2) size numpy array
		shape = face_utils.shape_to_np(shape)

		# Draw a rectangle over the detected face - vẽ khung
		...
		# tính tỉ lệ khung hình cho cả 2 mắt
		...
		# xác định khu vực của 2 mắt - tránh va chạm
		leftEyeHull = cv2.convexHull(leftEye)
		rightEyeHull = cv2.convexHull(rightEye)
		# Draw the contours - vẽ đường viền
		cv2.drawContours(frame, [leftEyeHull], -1, (0, 255, 0), 1)
		cv2.drawContours(frame, [rightEyeHull], -1, (0, 255, 0), 1)
		cv2.drawContours(frame, [mouth], -1, (0, 255, 0), 1)

		MAR = mouth_aspect_ratio(mouth)#tỉ lệ khuôn miệng
